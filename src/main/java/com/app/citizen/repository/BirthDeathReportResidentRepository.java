...
public interface BirthDeathReportResidentRepository extends JpaRepository<BirthDeathReportResident, BirthDeathReportResident.Pk> {
  @Query("select new com.app.citizen.domain.BirthDeathReportResidentDto(brR.birthReportQualificationsCode, rR.name, rR.residentRegistrationNumber, brR.phoneNumber, brR.emailAddress, brR.birthDeathReportDate) from BirthDeathReportResident brR join brR.reportResident rR where (brR.pk.residentSerialNumber = :id and brR.pk.birthDeathTypeCode = '출생')")
  public List<BirthDeathReportResidentDto> findBirthDtoBy(@Param("id") int id);


  @Query("select new com.app.citizen.domain.BirthDeathReportResidentDto(brR.deathReportQualificationsCode, rR.name, rR.residentRegistrationNumber, brR.phoneNumber, brR.emailAddress, brR.birthDeathReportDate) from BirthDeathReportResident brR join brR.reportResident rR where (brR.pk.residentSerialNumber = :id and brR.pk.birthDeathTypeCode = '사망')")
